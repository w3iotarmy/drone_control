...
from Constant import Server,CommandConstant
from Utility import ProjectCommand,SdCard
    ...
    while mpstate.status.exit != True:
        try:
            if mpstate.status.exit != True:
                ...
                #print ("Borhan ",mpstate.rl.prompt)
        except EOFError:
            mpstate.status.exit = True
            sys.exit(1)
        #print("borhan user input", line)
        ...
        try:
            ...
            if(sender_user=='ground'):
                ...
                if (val == CommandConstant.CommandConstantClass.is_equal_wp()):
                    ...
                    curr_alt = mpstate.status.altitude
                    print ("current alt ",curr_alt)
                    print("this is way point", lat, " ", lng)
                    wp_data = obj_data['data']
                    SdCard.SdCardClass.file_write(wp_data)

                elif (val == CommandConstant.CommandConstantClass.is_equal_arm()):
                    print ('command arm')
                    set_command(ProjectCommand.CommandClass.get_arm_string(1))
            elif sender_user=='eagle':
                print ("self message")
            ...
            print (error)
        #set_command(data)
    my_client.on('chat message', socker_receiver)
    my_client.wait()
#{"action":"wp","data":""}
if __name__ == '__main__':
    thread = threading.Thread(target=init_socket)
    thread.daemon = True
    thread.start()
    ...
    parser.add_option("--auto-protocol", action='store_true', default=False,
                      help="Auto detect MAVLink protocol version")
    parser.add_option("--mavversion", type='choice', choices=['1.0', '2.0'],
                      help="Force MAVLink Version (1.0, 2.0). Otherwise autodetect version")
    parser.add_option("--nowait", action='store_true', default=False, help="don't wait for HEARTBEAT on startup")
    parser.add_option("-c", "--continue", dest='continue_mode', action='store_true', default=False,
                      help="continue logs")
    ...
    parser.add_option("--default-modules",
                      default="log,signing,wp,rally,fence,param,relay,tuneopt,arm,mode,calibration,RC,auxopt,misc,cmdlong,battery,terrain,output,adsb",
